...
| Core + API smoke + hardening + perf + go-live tests | `npx vitest run --config tests/vitest.config.ts` | 98 pass + 1 conditional skip (first-run wizard, needs live fresh server) |
...
| Go-live deployment scenario | `tests/go-live.test.ts` — realistic shop end-to-end on a FILE db: setup→products→purchase→barcode sale→credit sale→return→dues→expenses→MFS→reconciliation (dashboard=P&L=ledger)→backup→concurrency→monitor enable/read-only→**application-restart persistence** | 15/15 pass |
