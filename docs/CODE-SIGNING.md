...
**已确认（2026-09-17）：** 发布者身份 = **经营者个人/sole proprietorship**（非公司实体）；签名路线待法律名称确定后再选定。

对个人身份的现实路线（Azure Artifact Signing 个人身份仅限美/加，不可用）：
- **Certum Standard Code Signing (Cloud)** — 面向自然人的云签名证书，全球可申请，~€25–75/年区间；发布者显示个人法定姓名；
- **SSL.com eSigner 个人 OV 代码签名** — 全球可申请，~$249/年起（含云签名订阅）；
- 两者都在 CA 云 HSM 中保存私钥（符合 2023 基线要求），提供 CI 可用的命令行工具（在 `scripts/sign-win.cjs` 的钩子点接入，选定服务后实现，约 20 行）。

需要经营者提供：
1. **法定英文姓名/经营者名称的确切写法**（与护照/NID 或 trade license 英文一致 — 将显示在 Windows"发布者"字段）；
2. 从上述两家之一购买个人代码签名证书（NID/护照验证）；
3. 添加 CI secrets（服务选定后给出精确变量名）。

> 公司/组织路线（OV + Azure Key Vault + AzureSignTool，见下）仍然保留：若未来注册公司，可无缝切换，发布者随之显示公司名。
